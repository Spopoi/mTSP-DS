...
class MTSP_DS_MILP_Solver(MTSP_DS_Solver):
    ...
    def set_model_constraints(self):
        ...
        # Constraint (5.1)
        self.model.addConstrs((gp.quicksum(self.x_k_ij[k, 0, j] for j in customer_ds_indexes) == 1 for k in self.K),
                              name="(5.1)")
        # Constraint (5.2)
        self.model.addConstrs(
            (gp.quicksum(self.x_k_ij[k, i, 1 + self.n + self.m] for i in customer_ds_indexes) == 1 for k in self.K),
            name="(5.2)")
        # Constraint (6)
